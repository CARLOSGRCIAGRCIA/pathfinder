...
describe('ObstacleController', () => {
  ...
  describe('createObstacle', () => {
    it('should create a single obstacle successfully', async () => {
      ...
      mockObstacleService.createObstacles.mockResolvedValue(Either.right([mockObstacle]));

      await ObstacleController.createObstacle(mockObstacleService)(mockReq, mockRes, mockNext);

      expect(mockObstacleService.createObstacles).toHaveBeenCalledWith('map123', mockReq.body);
    ...
    it('should create multiple obstacles successfully', async () => {
      ...
      mockObstacleService.createObstacles.mockResolvedValue(Either.right(mockObstacles));

      mockReq.body = mockObstacles;

      await ObstacleController.createObstacle(mockObstacleService)(mockReq, mockRes, mockNext);

      expect(mockObstacleService.createObstacles).toHaveBeenCalledWith('map123', mockObstacles);
      expect(mockRes.status).toHaveBeenCalledWith(201);
  ...
  describe('getObstacles', () => {
    it('should get obstacles successfully', async () => {
      ...
      mockObstacleService.getObstaclesByMapId.mockResolvedValue(Either.right(mockObstacles));

      await ObstacleController.getObstacles(mockObstacleService)(mockReq, mockRes, mockNext);

      expect(mockObstacleService.getObstaclesByMapId).toHaveBeenCalledWith('map123');
      expect(mockRes.json).toHaveBeenCalledWith(mockObstacles);
    });

    it('should handle empty obstacles list', async () => {
      mockObstacleService.getObstaclesByMapId.mockResolvedValue(Either.right([]));
    ...
    it('should handle map not found when getting obstacles', async () => {
      ...
      mockObstacleService.getObstaclesByMapId.mockResolvedValue(Either.left(error));
  ...
  describe('updateObstacle', () => {
    it('should update obstacle successfully', async () => {
      ...
      mockObstacleService.updateObstacle.mockResolvedValue(Either.right(updatedObstacle));

      await ObstacleController.updateObstacle(mockObstacleService)(mockReq, mockRes, mockNext);

      expect(mockObstacleService.updateObstacle).toHaveBeenCalledWith('obstacle123', mockReq.body);
  ...
  describe('deleteObstacle', () => {
    it('should delete obstacle successfully', async () => {
      ...
      await ObstacleController.deleteObstacle(mockObstacleService)(mockReq, mockRes, mockNext);

      expect(mockObstacleService.deleteObstacle).toHaveBeenCalledWith('obstacle123');
