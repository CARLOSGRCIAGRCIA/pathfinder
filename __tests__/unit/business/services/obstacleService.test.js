...
describe('ObstacleService', () => {
  ...
  describe('createObstacles', () => {
    it('should successfully create a single obstacle', async () => {
      ...
      DefaultObstacleStrategy.createMultiple.mockResolvedValue(Either.right([createdObstacle]));

      const result = await obstacleService.createObstacles(mockMapId, mockObstacleData);

      expect(result.isRight()).toBe(true);
      expect(DefaultObstacleStrategy.createMultiple).toHaveBeenCalledWith(mockMapId, [
        mockObstacleData,
      ]);
      result.fold(
    ...
    it('should successfully create multiple obstacles', async () => {
      ...
      DefaultObstacleStrategy.createMultiple.mockResolvedValue(Either.right(obstaclesData));

      const result = await obstacleService.createObstacles(mockMapId, obstaclesData);

      expect(result.isRight()).toBe(true);
      expect(DefaultObstacleStrategy.createMultiple).toHaveBeenCalledWith(mockMapId, obstaclesData);
      result.fold(
        () => fail('Should not return left'),
        data => {
  ...
  describe('getObstaclesByMapId', () => {
    it('should successfully retrieve obstacles', async () => {
      ...
      DefaultObstacleStrategy.findByMapId.mockResolvedValue(Either.right(mockObstacles));

      const result = await obstacleService.getObstaclesByMapId(mockMapId);

      expect(result.isRight()).toBe(true);
      expect(DefaultObstacleStrategy.findByMapId).toHaveBeenCalledWith(mockMapId);
      result.fold(
        () => fail('Should not return left'),
        data => {
    ...
    it('should handle retrieval errors', async () => {
      DefaultObstacleStrategy.findByMapId.mockResolvedValue(Either.left(new Error('Not found')));
  ...
  describe('updateObstacle', () => {
    it('should successfully update an obstacle', async () => {
      ...
      DefaultObstacleStrategy.update.mockResolvedValue(Either.right(updatedObstacle));

      const result = await obstacleService.updateObstacle(mockObstacleId, mockObstacleData);

      expect(result.isRight()).toBe(true);
      expect(DefaultObstacleStrategy.update).toHaveBeenCalledWith(mockObstacleId, mockObstacleData);
      result.fold(
        () => fail('Should not return left'),
        data => {
    ...
    it('should handle update errors', async () => {
      DefaultObstacleStrategy.update.mockResolvedValue(Either.left(new Error('Update failed')));
  ...
  describe('deleteObstacle', () => {
    it('should successfully delete an obstacle', async () => {
      DefaultObstacleStrategy.delete.mockResolvedValue(Either.right({ success: true }));

      const result = await obstacleService.deleteObstacle(mockObstacleId);

      expect(result.isRight()).toBe(true);
      expect(DefaultObstacleStrategy.delete).toHaveBeenCalledWith(mockObstacleId);
      result.fold(
        () => fail('Should not return left'),
        data => {
    ...
    it('should handle deletion errors', async () => {
      DefaultObstacleStrategy.delete.mockResolvedValue(Either.left(new Error('Delete failed')));
