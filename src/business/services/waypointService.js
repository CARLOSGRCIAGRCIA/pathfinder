...
const createWaypointService = waypointStrategy => ({
  createWaypoints: async (mapId, waypointData) => {
    const waypointsData = Array.isArray(waypointData) ? waypointData : [waypointData];

    const result = await waypointStrategy.createMultiple(mapId, waypointsData);

    return result.fold(
      error => Either.left(AppError(error.message || 'Error creating waypoints', 500)),
  ...
  getWaypointsByMapId: async mapId => {
    ...
    return result.fold(
      error => Either.left(AppError(error.message || 'Error fetching waypoints', 500)),
  ...
  updateWaypoint: async (waypointId, waypointData) => {
    ...
    return result.fold(
      error => Either.left(AppError(error.message || 'Error updating waypoint', 500)),
      data => Either.right(data)
    );
  },
  ...
  deleteWaypoint: async waypointId => {
    ...
    return result.fold(
      error => Either.left(AppError(error.message || 'Error deleting waypoint', 500)),
      data => Either.right(data)
    );
  },
});
