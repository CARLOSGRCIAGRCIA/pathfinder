...
const isWithinBounds = (map) => (x, y) =>
  x >= 0 && x < map.width && y >= 0 && y < map.height;

const validateDimensions = (map) =>
  map.width >= 50 && map.width <= 500 && map.height >= 50 && map.height <= 500
    ? Either.right(map)
    : Either.left(AppError('Map dimensions must be between 50 and 500', 400));
...
const validatePoint = (map) => (point, name) =>
  isWithinBounds(map)(point.x, point.y)
    ...
    : Either.left(
        AppError(
          `${name} point at (${point.x}, ${point.y}) is out of bounds`,
          400
        )
      );
  ...
  return requiredFields.reduce(
    (acc, field) =>
      acc.flatMap(() =>
        field in data
          ? Either.right(data)
          : Either.left(AppError(`Missing required field: ${field}`, 400))
      ),
    Either.right(data)
  );
};
...
const validateMapData = (mapData) =>
  validateRequiredFields(mapData).flatMap(() =>
    validateMapConfiguration(mapData)
  );
