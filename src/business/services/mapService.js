...
const MapService = (mapRepository, searchStrategy) => ({
  getMaps: async ({ page, limit, name, location }, userId) => {
    ...
    return result.fold(
      error => Either.left(AppError('Error fetching maps', 500)),
  ...
  searchMaps: async (query) => {
    ...
    return result.fold(
      error => Either.left(AppError('Error searching maps', 500)),
      data => Either.right(data)
    );
  },
  ...
  getMap: async (id, userId) => {
    ...
    return result.fold(
      error => Either.left(AppError(error.message || 'Map not found or you have no permission to access this map', 404)),
      data => Either.right(data)
    );
  },
  ...
  updateMap: async (id, mapData, userId) => {
    ...
    return result.fold(
      error => Either.left(AppError(error.message || 'Error updating map or you have no permission to update this map', 404)),
      data => Either.right(data)
    );
  },
  ...
  deleteMap: async (id, userId) => {
    ...
    return result.fold(
      error => Either.left(AppError(error.message || 'Error deleting map or you have no permission to delete this map', 404)),
      data => Either.right(data)
    );
  }
});
